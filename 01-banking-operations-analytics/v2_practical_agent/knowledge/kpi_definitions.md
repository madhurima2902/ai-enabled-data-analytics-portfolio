...
Average Transactions per Active Customer measures the average number of trusted warehouse transactions generated by customers who had at least one transaction during the reporting month.

For this approved metric, the words `user` and `customer` may be treated as synonyms in a natural-language question. The denominator is still defined as an active customer, not the full customer base.

### Formula

Average Transactions per Active Customer =  
Monthly Transaction Count / Distinct Active Customers in the Month

### Numerator

The numerator is the number of trusted warehouse transaction rows in the reporting month.

### Denominator

The denominator is the number of distinct `customer_id` values with at least one trusted warehouse transaction in the same reporting month.

### Business Meaning

A higher value means active customers generated more transactions on average during that month. A lower value means transaction activity per active customer was lower.

This metric does not use all registered customers as the denominator. If analysis requires transactions per total customer base, that must be defined as a separate approved metric rather than inferred automatically.
